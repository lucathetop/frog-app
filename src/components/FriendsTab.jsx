  ...
  const loadFriends = async () => {
    ...
    try {
      const { data } = await supabase
        .from("friendships")
        .select("*, requester:requester_id(username), receiver:receiver_id(username)")
  ...
  const handleSearch = async () => {
    ...
    try {
      const { data } = await supabase
        ...
        .eq("username", searchId.trim().toLowerCase())
        .single();
      if (!data) { setError("No user found with that username"); }
      else if (data.id === user.id) { setError("That's you!"); }
      else { setSearchResult(data); }
    } catch(e) { setError("No user found with that username"); }
    setSearching(false);
  };
    ...
    <div style={{flex:1,overflowY:"auto",padding:"14px 14px 90px",
      ...
      <div style={{background:"#fff9f4",borderRadius:16,padding:"16px 18px",
        ...
        <div style={{fontSize:11,fontWeight:600,letterSpacing:"1.5px",
          textTransform:"uppercase",color:"#2c2318",marginBottom:8}}>your username</div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:22,
            fontWeight:700,color:"#4a9457"}}>
            {profile?.username}
          </div>
          <div style={{fontSize:12,color:"#8a7260"}}>share this with friends</div>
        </div>
      </div>
      ...
      <div style={{background:"#fff9f4",borderRadius:16,padding:"16px 18px",
        ...
        <div style={{display:"flex",gap:8}}>
          <input value={searchId} onChange={e=>{setSearchId(e.target.value);setError("");setSearchResult(null);}}
            onKeyDown={e=>e.key==="Enter"&&handleSearch()}
            placeholder="enter their username"
            style={{flex:1,background:"#faf7f2",border:"1.5px solid #ede5d8",borderRadius:12,
              padding:"11px 14px",fontSize:14,color:"#2c2318",outline:"none",
      ...
      <div style={{background:"#fff9f4",borderRadius:16,padding:"16px 18px",
        ...
        {loading ? (
          ...
          <div style={{fontSize:13,color:"#8a7260",lineHeight:1.6}}>
            No friends yet — add some above!<br/>
            Share your username so they can find you.
          </div>
        ) : (
          friends.map(f => (
